...
export default function PatientMeasureAddScreen() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (dateObj) payload.date = dateObj.toISOString();
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ width: 32 }} />{/* placeholder pour aligner */}
      </View>

      <Text style={styles.question}>Quel type de mesure{"\n"}souhaitez-vous ajouter ?</Text>

      <View style={styles.chipsRow}>
        {["Glycémie", "Tension", "Poids", "Température"].map((label, index) => {
          const key = label.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, "");
          const t: MeasureType = (["glycemie", "tension", "poids", "temperature"] as MeasureType[])[index];
          const active = type === t;
          return (
            <TouchableOpacity
              key={key}
      ...
      <View style={styles.group}>
        ...
        <TouchableOpacity style={styles.dateBox} onPress={() => setShowDatePicker(true)}>
          <Text style={styles.dateText}>{formatDisplayDate(dateObj)}</Text>
        ...
        {showDatePicker && (
          <DateTimePicker
            ...
            display="default"
            onChange={(e, selected) => {
              onChangeDate(e, selected);
              if (Platform.OS !== "ios") setShowTimePicker(true); // open time on android after date
            }}
          />
        )}
        {showTimePicker && (
        ...
        {/* small row to open time separately */}
        <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
          ...
          >
            <Text style={styles.smallAction}>Modifier la date</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => {
          ...
          >
            <Text style={styles.smallAction}>Modifier l'heure</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.group}>
        ...
        <TextInput
          ...
          placeholder={type === "tension" ? "ex: 120/80" : "ex: 90"}
          keyboardType={type === "tension" ? "default" : "numeric"}
        />
